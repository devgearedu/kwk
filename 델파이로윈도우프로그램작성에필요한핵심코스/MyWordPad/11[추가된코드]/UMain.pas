...
uses
  ...
  Vcl.BaseImageCollection, Vcl.ImageCollection,Vcl.OleAuto,System.Win.ScktComp;
...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure About_Dynamic_ActionExecute(Sender: TObject);
    procedure About_Static_ActionExecute(Sender: TObject);
    procedure About_Package_ActionExecute(Sender: TObject);
...
procedure TMainForm.About_Dynamic_ActionExecute(Sender: TObject);
begin
   H := LoadLibrary('PCalc.dll');
  if h < 32  then
     raise Exception.Create('그런 라이브러리가 없음 이름과 경로확인');


//  SubFunc :=  GetProcAddress(h, 'Sub');
//  ShowMessage(IntToStr(SubFunc(12,3)));


  DivFunc  := GetProcAddress(h, 'Divide');
  ShowMessage(FloatToStr(DivFunc(12.8,6.4)));

  FreeLibrary(h);
end;

procedure TMainForm.About_Package_ActionExecute(Sender: TObject);
var
...
begin
  try
     ...
     on EFopenError do
        ShowMessage('그런 파일 없습니다 파일이름과 패스 확인');
...
begin
   ...
   FilePath := ExtractFilePath(Application.ExeName);
   FileName :=
  'E:\델파이로윈도우프로그램작성에필요한핵심코스\MyHandsOn\14\Win32\Debug\MyWordPad.exe';
